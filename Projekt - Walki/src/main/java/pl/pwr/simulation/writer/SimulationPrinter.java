...
public final class SimulationPrinter {
    ...
    public void print(Squad squad, Map<Squad, Coordinates> squadLocation){
        System.out.println
                ("\n------------------------------------------------------------" + "\nID: " + squad.getSquadId() + "\n" + squad.getName() + " are about to move."
                + "\nTheir coordinates are: " + squadLocation.get(squad).toString()
                + "\nAmount of units: " + squad.getSquadQuantity()
                + "\nDamage: " + squad.getDamage()
                + "\nMelee: " +squad.getMeleeDamage()
                + "\nRanged: " +squad.getArchersDamage()
    ...
    public void printMap(Field field, SimulationMap simulationMap){
        System.out.print("  ");
        ...
        for(int x = 0; x < field.getSize(); x++){
            for (int y = 0; y < field.getSize() + 3; y++){
                ...
                else if (field.getFieldProperties()[x][y - 2] == 3 && simulationMap.getSquadMap()[x][y - 2].getSquadId() != -1) System.out.print(simulationMap.getSquadMap()[x][y - 2].getSquadId());
