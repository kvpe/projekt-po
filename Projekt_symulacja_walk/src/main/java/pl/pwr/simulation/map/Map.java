...
public class Map
{
    ...
    public Map(Field field, int maxSquadSize, int amountOfSquads) throws IOException {
        ...
        SquadSpawnPointGenerator spawnPointGenerator = new SquadSpawnPointGenerator(amountOfSquads);
    ...
    public Squad checkSquadsInRange(Squad squad) {
        ...
        iStart = x - squad.getArcher().getRange();
        iEnd = x + squad.getArcher().getRange();

        if (x - squad.getArcher().getRange() < 0) iStart = 0;
        if (x + squad.getArcher().getRange() > squadMap.length) iEnd = squadMap.length;

        for (; iStart < iEnd; iStart++) {
            jStart = y - squad.getArcher().getRange();
            jEnd = y + squad.getArcher().getRange();
            if (y - squad.getArcher().getRange() < 0) jStart = 0;
            if (y + squad.getArcher().getRange() > squadMap.length) jEnd = squadMap.length;
