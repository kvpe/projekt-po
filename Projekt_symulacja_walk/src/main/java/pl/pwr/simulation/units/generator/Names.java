...
public class Names {
    ...
    public Names() throws IOException {
        Scanner reader = new Scanner(new File("E:\\IntelliJ Java projekty\\Projekt_symulacja_walk\\src\\main\\resources\\firstPartOfName.txt"));
        while(reader.hasNextLine()){
            names1.add(reader.nextLine());
        }
        reader.close();
        reader = new Scanner(new File("E:\\IntelliJ Java projekty\\Projekt_symulacja_walk\\src\\main\\resources\\secondPartOfName.txt"));
        while(reader.hasNextLine()){
