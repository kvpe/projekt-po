...
public class Squad implements FightResolver, RandomEventsInterface{
    ...
    @Override
    public int attack() {
        System.out.println(name + " id: "+ squadId + " has attacked a different squad!");
    ...
    public void deathFromStarvation() {
        ...
        for(; i <= 0; i++){
            if(horsemanQuantity > 0){
                horsemanQuantity--;
                System.out.println("Removing horseman due to hunger.");
                horsemanHealth = horsemanQuantity * (horseman.getHealth() + horseman.getHorseHealth());
            }
            else if(soldiersQuantity > 0){
                soldiersQuantity--;
                System.out.println("Removing soldier due to hunger.");
                soldiersHealth = soldiersQuantity * soldier.getHealth();
            }
            else if(archersQuantity > 0){
                archersQuantity--;
                System.out.println("Removing archer due to hunger.");
                archersHealth = archersQuantity * archer.getHealth();
            }
            setStats();
            if(squadQuantity == 0){
                System.out.println("Removing squad " + squadId + " as they starved to death.");
                removeSquad();
                break;
            }
        }
    ...
    public Coordinates move(Field field) {
        ...
        do{
            x = RandomGenerator.random.nextInt(moveRange) - RandomGenerator.random.nextInt(moveRange);
            y = RandomGenerator.random.nextInt(moveRange) - RandomGenerator.random.nextInt(moveRange);
        }while((x^2+y^2) > (moveRange^2) || (squadPosition.getX() + x < 0) || (squadPosition.getX() + x > (field.getSize() - 1)) || (squadPosition.getY() + y < 0) || (squadPosition.getY() + y  > (field.getSize()) - 1) );
    ...
    public Squad defence(Squad squad, int damage) {
        System.out.println(squad.name + " id: " + squadId + " has been attacked by a different squad!\nThey receive " + damage + " damage!");
    ...
    public void chooseUnitsToDie(int damage) {
        ...
        if (damage > 0) {
            ...
            if (archersHealth <= 0) {
                System.out.println(squadId + " has been defeated!");
                removeSquad();
            }
            else{
